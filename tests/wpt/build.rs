...
fn main() {
    ...
    eprintln!("cargo:rerun-if-changed=../../tests_wpt");
    eprintln!("cargo:rerun-if-changed={CONFIG_PATH}");
    ...
    if std::env::var("WPT_ROOT").is_err() {
        eprintln!("cargo:rerun-if-changed={root}");
        eprintln!("cargo:rustc-env=WPT_ROOT={root}");
    }
}
